...
def normalize_history_item(prompt_id: str, history_item: dict, include_outputs: bool = False) -> dict:
    ...
    if execution_summary or completion_status is not None:
        has_errors = execution_summary.get('has_errors', bool(execution_errors))
        execution_error_count = execution_summary.get('execution_error_count', len(execution_errors))
    else:
        has_errors = None
