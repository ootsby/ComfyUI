...
class TestExpansionAroundFailingOutput(ComfyNodeABC):
    @classmethod
    def INPUT_TYPES(cls):
        return {"required": {"image": (IO.IMAGE, {})}}
    ...
    def expand(self, image):
        graph = GraphBuilder()
        inner = graph.node("TestExpansionWithFailingOutput", image=image)
        passthrough = graph.node("TestSleep", value=inner.out(0), seconds=0.0)
        return {
            "result": (passthrough.out(0),),
            "expand": graph.finalize(),
        }
...
ASYNC_TEST_NODE_CLASS_MAPPINGS = {
    ...
    "TestExpansionAroundFailingOutput": TestExpansionAroundFailingOutput,
...
ASYNC_TEST_NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    "TestExpansionAroundFailingOutput": "Test Expansion Around Failing Output",
